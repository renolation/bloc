...
class AuthRemoteDataSourceImpl implements AuthRemoteDataSource {
  ...
  Future<void> forgotPassword(String email) async {
    try {
      ...
      throw ServerException(message: e.message ?? 'Error occurred', statusCode: int.parse(e.code));
    } catch (e, s) {
  ...
  Future<LocalUserModel> signIn({required String email, required String password}) async {
    try {
      final result = await _authClient.signInWithEmailAndPassword(email: email, password: password);
      final user = result.user;
      if (user == null) {
        throw const ServerException(message: 'Please try again later', statusCode: 404);
      }
